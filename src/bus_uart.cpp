...
bool BusUART::setInputParam(HAL::IO_PARAM_t param, int32_t value) {
  ...
  default:
    Core::printE("Serial bus had incorrect input param type change. Param "
                 "type: %d, Value:%d.\n",
                 param, value);
    break;
  }
...
size_t BusUART::writable() { return bus_.availableForWrite(); }
...
size_t BusUART::readable() { return bus_.available(); }

size_t BusUART::readData(void *data, size_t size, bool endTransfer) {
  return bus_.readBytes((uint8_t *)data, size);
}

bool BusUART::writeRead(const void *writeBuf, void *readBuf, size_t writeSize,
                        size_t readSize, bool endTransfer) {
