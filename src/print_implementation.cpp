...
void printOutM(const char *const &item) { Serial.print(item); };
...
public:
  ...
  PlatformArduino()
      : VCTR::Core::Task_Periodic("Platform Arduino Task",
                                  1 * VCTR::Core::SECONDS, 0) {
  ...
  void taskThread() override { setRelease(VCTR::Core::END_OF_TIME); }
};
